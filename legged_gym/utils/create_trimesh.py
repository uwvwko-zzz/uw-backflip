...
def frame_trimesh(
    ...
    vertices[8, 1] -= size[1] / prop
    vertices[8, 2] += size[1] / prop
    vertices[9, 0] += size[0] / 2
    vertices[9, 1] -= size[1] / prop
    vertices[9, 2] += size[1] / prop
    vertices[10, 0] -= size[0] / 2
    vertices[10, 1] -= size[1] / prop
    vertices[10, 2] -= size[1] / prop
    vertices[11, 0] += size[0] / 2
    vertices[11, 1] -= size[1] / prop
    vertices[11, 2] -= size[1] / prop
    vertices[12, 0] -= size[0] / 2
    vertices[12, 1] += size[1] / prop
    vertices[12, 2] += size[1] / prop
    vertices[13, 0] += size[0] / 2
    vertices[13, 1] += size[1] / prop
    vertices[13, 2] += size[1] / prop
    vertices[14, 0] -= size[0] / 2
    vertices[14, 1] += size[1] / prop
    vertices[14, 2] -= size[1] / prop
    vertices[15, 0] += size[0] / 2
    vertices[15, 1] += size[1] / prop
    vertices[15, 2] -= size[1] / prop
...
def QRC_trimesh(center_pos,obj_path, prop=1):
    obj_mesh = trimesh.load(obj_path)
    #print('stepover_1 mesh shape is:',obj_mesh.vertices.shape[0])
    v = np.empty((obj_mesh.vertices.shape[0], 3), dtype= np.float32)
    v[:,] = center_pos
    v += obj_mesh.vertices.astype(np.float32)
    #v = obj_mesh.vertices
    f = obj_mesh.faces.astype(np.uint32, copy=False)  #must strictly obey the requirements of gym.add_triangle_mesh() function input
    #print('triangle data type:', f.dtype)
    v_ = np.array(v)
    f_ = np.array(f)
    return v_, f_    
